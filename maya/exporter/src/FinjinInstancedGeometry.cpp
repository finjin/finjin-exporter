...
MStatus FinjinInstancedGeometry::compute(const MPlug& plug, MDataBlock& data)
{
    return MStatus::kSuccess;
}

void FinjinInstancedGeometry::DrawLocator(M3dView& view, M3dView::DisplayStyle style, M3dView::DisplayStatus status)
{
...
MStatus FinjinInstancedGeometry::Initialize()
{
    return AddAttributes() == MStatus::kSuccess ? MStatus::kSuccess : MS::kFailure;
}

void* FinjinInstancedGeometry::Creator()
{
    return new FinjinInstancedGeometry();
}
...
MStatus FinjinInstancedGeometry::AddAttributes()
{
    ...
    attributes.batchCount = adder.AddInt("BatchCount", 1, 1, 100);
